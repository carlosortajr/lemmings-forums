...
function MessagePost2()
{
	...
	if (!empty($recipientList['to']) || !empty($recipientList['bcc']))	
		$context['send_log'] = sendpm($recipientList, $_REQUEST['subject'], $_REQUEST['message'], !empty($_REQUEST['outbox']), null, !empty($_REQUEST['pm_head']) ? (int) $_REQUEST['pm_head'] : 0, empty($attachIDs) ? array() : $attachIDs);
	else
		$context['send_log'] = array(
			'sent' => array(),
			'failed' => array()
